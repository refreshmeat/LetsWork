...
function applicationFormEvidence(page){
  ...
  return /<form\b/i.test(h)&&(/type=["']file["']/i.test(h)||/name=["'][^"']*(?:curriculo|resume|cv)[^"']*["']/i.test(h)||/textarea/i.test(h));
}
async function followApplication(url,depth=0){
  ...
  if(applicationFormEvidence(page))return {sendable:1,reason:''};
  if(depth<1){
...
export async function probeJobSendability(job){
  ...
  if(source==='Gupy'||source==='Vagas.com'){
    ...
    return applicationFormEvidence(first)?{sendable:1,reason:'',verified:true}:{sendable:0,reason:'UNVERIFIED_LOGIN',verified:false};
  }
  if(applicationFormEvidence(first))return {sendable:1,reason:'',verified:true};
  if(links.length){const r=await followApplication(links[0]);return {...r,verified:r.reason!=='UNVERIFIED_LOGIN'};}
  return {sendable:0,reason:'UNVERIFIED_LOGIN',verified:false};
